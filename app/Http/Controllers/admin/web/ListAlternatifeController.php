...
class ListAlternatifeController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $collection = ListAlternatife::join('list_food','id_list_food', '=', 'list_food.id')->join('list_day','id_list_day', '=', 'list_day.id')->select('list_day.day','list_alternatife.id','list_day.date','list_food.name_food','list_alternatife.description')->paginate(20);
